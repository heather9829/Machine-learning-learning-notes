...
> **前向搜索**：特征子集X从空集开始，每次选择一个特征x加入特征子集X，使得特征函数J( X)最优。
> **后向搜索**：从特征全集O开始，每次从特征集O中剔除一个特征x，使得剔除特征x后评价函数值达到最优。
...
直观上理解：对于猜中近邻，两者j属性的距离越小越好，对于猜错近邻，j属性距离越大越好。更一般地，若xi为离散属性，diff取海明距离(在信息编码中，两个合法代码对应位上编码不同的位数称为码距，又称海明距离。举例如下：10101和00110从第一位开始依次有第一位、第四、第五位不同，则海明距离为3。)，即相同取0，不同取1；若xi为连续属性，则diff为曼哈顿距离，即取差的绝对值。分别计算每个分量，最终取平均便得到了整个相关统计量。
...
LVW算法的具体流程如下所示，假设数据集为 D，特征集为 A，误差E，则 LVW 每次从特征集 A 中随机产生一个特征子集 A′，然后使用交叉验证的方法（伪代码的第 8 步）估计学习器在特征子集 A′ 上的误差，若该误差小于之前获得的最小误差E′，或者与之前的最小误差相当E=E′但 A′ 中包含的特征数更少，则将 A′ 保留下来。 
...
（2）**空间限制**：L1范数与L2范数都试图在最小化损失函数的同时，让权值W也尽可能地小。我们可以将原优化问题看做为下面的问题，即让后面的规则则都小于某个阈值。这样从图中可以看出：L1范数相比L2范数更容易得到稀疏解。(与L1的交点在坐标轴上，即w1,w2=0)
